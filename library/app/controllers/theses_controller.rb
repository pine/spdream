...
class ThesesController < ApplicationController
	...
	def all
		@theses = Thesis.find(:all, :conditions => { :enabled => true }, :order => "year DESC, student_id ASC")
		@faculties = Faculty.all
	end

	def faculty
		@theses = Thesis.find(:all, :conditions => { :enabled => true, :faculty_id => params[:id] }, :order => "year DESC, student_id ASC")
		@labs = Lab.find(:all, :conditions => {:faculty_id => params[:id]})
		@faculty = Faculty.find(params[:id])
	end

	def lab
		@theses = Thesis.find(:all, :conditions => { :enabled => true, :faculty_id => params[:faculty_id], :lab_id => params[:lab_id] }, :order => "year DESC, student_id ASC")
